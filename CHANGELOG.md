...
- **Vertical scrollbar thumb can no longer be dragged past the track bottom.**
  ...
  vertical track stops above the horizontal lane and the thumb's `maxOffset`
  matches `updateVScroll`'s scroll clamp (they previously disagreed by one
  gutter whenever both scrollbars were shown).
