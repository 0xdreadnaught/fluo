...
func TestWrapFlows(t *testing.T) {
	...
	if got := w.DesiredSize(); got != (render.Size{W: 120, H: 40}) {
		t.Fatalf("desired=%v", got)
	}
	core.ArrangeWidget(w, render.Rect{X: 0, Y: 0, W: 130, H: 500})
	if got := kids[1].Bounds(); got != (render.Rect{X: 60, Y: 0, W: 60, H: 20}) {
		t.Fatalf("k1=%v", got)
	}
	if got := kids[2].Bounds(); got != (render.Rect{X: 0, Y: 20, W: 60, H: 20}) {
		t.Fatalf("k2 wrapped=%v", got)
	}
}
...
func TestWrapGap(t *testing.T) {
	...
	if got := w.DesiredSize(); got != (render.Size{W: 130, H: 20}) {
		t.Fatalf("desired=%v", got)
	}
}
...
func TestWrapOversizeChildGetsOwnRow(t *testing.T) {
	...
	if got := w.DesiredSize().H; got != 20 {
		t.Fatalf("H=%v (two rows)", got)
	}
}
