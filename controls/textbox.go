...
func (t *TextBox) vScrollTrack() (render.Rect, bool) {
	...
	// Track height is the CONTENT viewport height (contentHeight), which is
	// shortened by the horizontal lane when the h-bar shows — so the vertical
	// track stops above the h-lane (leaving the corner) AND its thumb math
	// (maxOffset = totalContentHeight - track.H) matches updateVScroll's clamp
	// (maxScroll = totalContentHeight - innerH). Otherwise the two disagree by
	// one gutter and the thumb can be dragged past the track's bottom.
	h := t.contentHeight()
	if h <= 0 {
		return render.Rect{}, false
	}
